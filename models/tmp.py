...
from models.kv_utils import init_ALLKV
...
class LlamaAttention_KIVI(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            # past_key_value = self.kv_cluster.update_quant_kv_in_decoding(query_states=query_states, past_key_value=past_key_value, group_size=self.group_size, bits=self.k_bits, num_key_value_groups=self.num_key_value_groups)
            # print(query_states.shape, key_states.shape, value_states.shape)
            ...
            kv_cache_len = key_states_full.shape[-2] if key_states_full is not None else 0
            kv_cache_len += self.group_size * (key_mn_trans.shape[-1] if key_mn_trans is not None else 0)
            kv_cache_len += key_states.shape[-2]
            # pkv[0]=torch.Size([1, 8, 64, 64])
            # pkv[1]=torch.Size([1, 8, 125, 64])
            # pkv[2]=torch.Size([1, 8, 64, 32])
            # pkv[3]=torch.Size([1, 8, 64, 32])
            # pkv[4]=torch.Size([1, 8, 1021, 4])
            # pkv[5]=torch.Size([1, 8, 128, 64])
            # pkv[6]=torch.Size([1, 8, 1021, 2])
            # pkv[7]=torch.Size([1, 8, 1021, 2])
            # pkv[8]=torch.Size([1, 8, 64, 32])
            # if cnt <= 9:
            #     for i in range(9):
            #         if past_key_value[i] is not None:
            #             print(f"pkv[{i}]={past_key_value[i].shape}") 
            #     cnt += 1
            ...
            if key_states_full.shape[-2] == self.residual_length:
                ...
                key_states_quant_trans_new, key_scale_trans_new, key_mn_trans_new, key_mx_trans_new = triton_quantize_and_pack_along_last_dim(key_states_full.transpose(2, 3).contiguous(), 
                                                                                                                            self.group_size, 
                                                                                                                            self.k_bits)
            ...
            if value_states.shape[-2] != 1:
                print("value_state_full, value_states")
                print(value_states_full.shape, value_states.shape)
            ...
            if value_full_length > self.residual_length:
                # print(value_full_length)
                # print(self.residual_length)
                assert value_full_length == self.residual_length + 1
                value_states_quant_new, scale, mn, *_ = triton_quantize_and_pack_along_last_dim(value_states_full[:, :, :1, :].contiguous(), 
                                                                                                self.group_size, 
                                                                                                self.v_bits)
        ...
        # prefill 
            ...
            # key_states = key_states[:, :, -3548:, :]
            # value_states = value_states[:, :, -3548:, :]
            if(attention_mask is not None):
                key_states, value_states, attention_mask = self.kv_cluster.update_kv_snap(key_states, query_states, value_states, attention_mask)
            else:
                key_states, value_states = self.kv_cluster.update_kv_snap(key_states, query_states, value_states)
            # print(key_states.shape, value_states.shape, query_states.shape, attention_mask.shape)
            ...
            if key_states_quant is not None:
                key_states_quant_trans, key_scale_trans, key_mn_trans, key_mx_trans = triton_quantize_and_pack_along_last_dim(key_states_quant.transpose(2, 3).contiguous(), self.group_size, self.k_bits)
            else:
            ...
            if value_states.shape[-2] <= self.residual_length:
                ...
                value_states_quant, value_scale, value_mn, *_ = triton_quantize_and_pack_along_last_dim(value_states_quant, 
                                                                                                self.group_size, 
                                                                                                self.v_bits)
            ...
            # print(key_states.shape, value_states.shape, attn_weights.shape, self.num_key_value_groups)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            # if key_states_full.shape[-2] == self.residual_length:
            #     assert self.residual_length % self.group_size == 0
            #     key_states_quant_trans_new, key_scale_trans_new, key_mn_trans_new,  key_mx_trans_new, *_= triton_quantize_and_pack_along_last_dim(key_states_full.transpose(2, 3).contiguous(), 
            #                                                                                                                 self.group_size, 
            #                                                                                                                 self.k_bits)
            #     key_states_full = None
            #     if key_states_quant_trans is not None:
            ...
            #         key_scale_trans = torch.cat([key_scale_trans, key_scale_trans_new], dim=3)
            #         key_mn_trans = torch.cat([key_mn_trans, key_mn_trans_new], dim=3)
            #         key_mx_trans = torch.cat([key_mx_trans, key_mx_trans_new], dim=3)
            #     else:
            #         key_states_quant_trans = key_states_quant_trans_new
            #         key_scale_trans = key_scale_trans_new
            #         key_mn_trans = key_mn_trans_new
            ...
            # if attn_weights.size() != (bsz, self.num_heads, q_len, key_states.shape[-1]):
            #     raise ValueError(
            ...
            # key_states, value_states, attention_mask = self.kv_cluster.update_kv_snap(key_states, query_states, value_states, attention_mask)
            if(key_states.shape[-2] > 100):
                half = key_states.shape[-2] // 2
                key_states = torch.cat([key_states[:, :, :half - 5, :], key_states[:, :, -half + 5:, :]], dim = -2)
                value_states = torch.cat([value_states[:, :, :half - 5, :], value_states[:, :, -half + 5:, :]], dim = -2)
            #     key_states, value_states = key_states[:, :, 100:, :], value_states[:, :, 100:, :]
            ...
            #     key_states_full = None
            # if key_states_quant is not None:
            #     key_states_quant_trans, key_scale_trans, key_mn_trans, key_mx_trans, *_ = triton_quantize_and_pack_along_last_dim(key_states_quant.transpose(2, 3).contiguous(), self.group_size, self.k_bits)
            # else:
            #     key_states_quant_trans = None
            #     key_scale_trans = None
            #     key_mn_trans = None
